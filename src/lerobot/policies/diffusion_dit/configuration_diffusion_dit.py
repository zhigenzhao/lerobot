...
@PreTrainedConfig.register_subclass("diffusion_dit")
@dataclass
class DiffusionDiTConfig(PreTrainedConfig):
    """Configuration class for DiffusionDiTPolicy.
    ...
    This combines the efficiency of DiT's AdaLN with flexible cross-attention for observations.
    ...
    Args:
        ...
        horizon: Diffusion model action prediction size as detailed in `DiffusionDiTPolicy.select_action`.
        n_action_steps: The number of action steps to run in the environment for one invocation of the policy.
            See `DiffusionDiTPolicy.select_action` for more details.
        ...
        hidden_size: Hidden dimension for the DiT transformer blocks.
        ...
        patch_size: Patch size for patch embedding (if using patch-based tokenization).
        diffusion_step_embed_dim: The transformer is conditioned on the diffusion timestep via AdaLN.
            This is the embedding dimension for the timestep.
        use_adaln_zero: Whether to use AdaLN-Zero initialization for better training stability.
        cross_attention_dim: Dimension of the cross-attention layer for observation conditioning.
        add_cross_attention: Whether to add cross-attention layers for global conditioning.
        noise_scheduler_type: Name of the noise scheduler to use. Supported options: ["DDPM", "DDIM"].
        num_train_timesteps: Number of diffusion steps for the forward diffusion schedule.
        beta_schedule: Name of the diffusion beta schedule as per DDPMScheduler from Hugging Face diffusers.
        beta_start: Beta value for the first forward-diffusion step.
        beta_end: Beta value for the last forward-diffusion step.
        prediction_type: The type of prediction that the diffusion modeling transformer makes. Choose from "epsilon"
    ...
    def __post_init__(self):
        ...
        if self.diffusion_step_embed_dim <= 0:
            raise ValueError(f"`diffusion_step_embed_dim` must be positive. Got {self.diffusion_step_embed_dim}.")
